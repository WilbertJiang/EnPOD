...
u = Function(V)
for n in range(0,Train_steps):
    solution_load_file_name = "./solution_13blocks_pre1_2_update/file_" + str(n) + "h5"
...
for n_t in range(0, N_mode):
    ...
    for i in n_list:
    #for i in range(0,NU):
        CU_filename = 'pod_result/CU_'+str(i)+'/CU'+str(n_t +1)+".txt"
    ...
    for i in range (0,Train_steps):
        ...
        Time_name = 'pod_result/T_overtime_'+str(n_t + 1)+'_mode.txt'
        Time_file = open(Time_name,'a')
        Time_file.write('%.16g\t' % (a((0.00361, 0.003155,5.85e-4))))
        Time_file.write('%.16g\n' % (solution[i]((0.00361, 0.003155,5.85e-4))))
        Time_file.close()
    ...
    points = [(0.00361, y_,5.85e-4) for y_ in y]  # 2D points
    ...
    points = [(y_,0.003155,5.85e-4) for y_ in y]
    w_line = np.array([a(point) for point in points])
    p_line = np.array([solution[num_steps-1](point) for point in points])
    if n_t == 0:
