...
u = Function(V)
for n in range(0,Train_steps):
    solution_load_file_name = "./solution_13blocks_pre1_2/file_" + str(n) + "h5"
        ...
        for j in range (0, n_t +1):
            if n_t ==0:
                for n_c in range(0, len(n_list)):
                    a.vector().axpy(CU[n_c][i], obj['podmode' + str(n_list(n_c))][j].vector())
            else:
                for n_c in range(0, len(n_list)):
                    a.vector().axpy(CU[n_c][i][j], obj['podmode' + str(n_list(n_c))][j].vector())
