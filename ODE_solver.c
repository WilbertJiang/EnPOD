/* import C, G, P_vector and temperatu:re gradient to solve ODE.  And then save the coefficient of ppodmode*/
#include "petscmat.h"
#include <slepceps.h>
#include <stdio.h>
#include <petscts.h>
...
char namep[200];
sprintf(namep, "P_matrix_pre1_2_update_%d.txt", N_index);
 ...
 TSSetRHSFunction(ts,NULL,RHSFunctionHeat,&appctx);
  ...
  TSSetSolution(ts,u0);
   ...
   for( int k = 0; k < num_pod; k++)
	...
	char namecu[200];
	sprintf(namecu, "pod_result/CU_%d/CU%d.txt",N_index,num_pod);
