...
MPI_Comm        comm;
PetscInt        *dnnz,*onnz,i,j,steps; // varible will be used in slepc
	 /*****************************************************************************/
IS             perm,iperm;
MatFactorInfo  info;
...
/****************************************************/ 
  ...
  */
 int NT = 4800, NC = 30, NP = 12, N_index = 0;
 N_index = atoi(argv[1]);
 printf("The index of functional unit is: %d\n",N_index);
...
double sampling_interval = 4.166666e-6; 
/***********************************************************
 *
...
char nameC[200];
sprintf(nameC, "C_matrix_%d.txt", N_index);
...
char nameG[200];
sprintf(nameG, "G_matrix_%d.txt", N_index);
FILE* fileG = fopen(nameG,"r");
for (int i = 0; i < NC; i++){
	for(j = 0; j < NC; j++){
		Gmatrix[i][j] = 0;
 ...
 * ***********************************************************/
double **PSmatrix = malloc(NP*sizeof(double*));
if (!PSmatrix)
        return -1;
for(int i = 0; i < NP; i++){
...
char namep[200];
sprintf(namep, "P_matrix_pre2_%d.txt", N_index);
FILE* filep = fopen(namep,"r");
for (int i = 0; i < NP; i++){
	for (int j = 0; j < NT; j++){
		PSmatrix[i][j]=0;
...
for(int i=0;i<NT;++i){
	...
	//int num_pod = NP;
	for(int num_pod = 1; num_pod < NP; num_pod++){
...
// create timestepping solver context
  ...
  MatDuplicate(CG, MAT_COPY_VALUES,&appctx.A);
   ...
   for (int i = 0; i < NT; i ++){
	...
	//printf("this works\n");
 	...
 	dt = sampling_interval; /* which should be change to sampling interval /20 in future */
  	TSSetTimeStep(ts,dt);
   ...
   */
   ...
   for( int k = 0; k < num_pod; k++)
	...
	char namecu[200];
	sprintf(namecu, "Computation_time/ODE_block%d.txt",N_index);
	FILE *fileMAT4 = fopen(namecu,"a");
	fprintf(fileMAT4,"%.16lg\n",cpu_time_used);
	fclose(fileMAT4);	
	TSDestroy(&ts);
	MatDestroy(&CP);
	MatDestroy(&RC);
