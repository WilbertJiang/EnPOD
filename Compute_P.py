...
pd = loadtxt('powertrace_AMD_MLB_240_pre5.txt')                                        #pd  is power density
#import floorplan file
#Line Format: <unit-name>\t<width>\t<height>\t<left-x>\t<bottom-y>\t[<specific-heat>]\t[<resistivity>]
#compute power desity
...
u = Function(V)
for n in range(0,N_mode):
    mode_load_file_name = "podmode_"+str(Nu2)+"/mode_" + str(n) + "h5"
...
for j in range(0, N_mode):
    ...
    for i in range(0, num_steps):
        print(i,":",j)
        P_matrix[j][i]=pd[i,Nu2]*value_P
header = 'P_matrix_pre5_'+str(Nu2)
