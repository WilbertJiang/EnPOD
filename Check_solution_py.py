...
u = Function(V)
n = 579
solution_load_file_name = "./solution_3_outof_13/file_" + str(n) + "h5"
...
for n_t in range(0, 1):
    ...
    if n_t ==0:
        ...
        for ii in range(0,len(y)):
            data_file.write('%.16g\n' % (y[ii]))
        data_file.close()
    header_data = 'pod_result/T_Y_'+str(n_t + 1)+'_mode_py'
    data_file_name = header_data + '.txt'
    data_file = open(data_file_name,'w')
    for ii in range(0,len(y)):
        data_file.write('%.16g\n' % (p_line[ii]))
    ...
    points = [(y_,0.009205,5.85e-4) for y_ in y]
    p_line = np.array([solution[n](point) for point in points])
    if n_t == 0:
        ...
        for ii in range(0,len(y)):
            data_file.write('%.16g\n' % (y[ii]))
        data_file.close()
    header_data = 'pod_result/T_X_'+str(n_t +1)+'_mode_py'
    data_file_name = header_data + '.txt'
    data_file = open(data_file_name,'w')
    for ii in range(0,len(y)):
        data_file.write('%.16g\n' % (p_line[ii]))
